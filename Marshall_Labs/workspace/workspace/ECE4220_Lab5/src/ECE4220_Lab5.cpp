	...
	while (1)
	{
		...
		if(message.compare("WHOIS") == 0){
			if(masterOrCommander == true){
				...
				n = sendto(sock, "Marshall Lindsay on " + myIp + " is the master!", MSG_SIZE, 0, (struct sockaddr *)&from, fromlen);
				//Error Check
				if(n < 0){
				...
				addr.sin_addr.s_addr = inet_addr(""); //broadcast address
				//Send the message
				n = sendto(sock, "# " + myIp + converted, MSG_SIZE, 0, (struct sockaddr*)&from, fromlen);
				//Error Check
				if(n < 0){
