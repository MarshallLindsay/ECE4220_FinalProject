...
void* readFromCharacterDevice(void*);
...
int main(int argc, char *argv[]) {
	...
	pthread_create(&charDevReader, NULL, readFromCharacterDevice, NULL);
	...
	while (1)
	{
		...
		//Check if master
		if(masterOrCommander){
			//Check if a button was pressed
			if(sem_trywait(&charDev_Semaphore)){
				//Broadcast the note
				cout<<"\nSending : "<<send<<endl;
				n = sendto(sock, &broadcast, MSG_SIZE, 0, (struct sockaddr*)&from, fromlen);
				//Error Check
				if(n < 0){
					cout<<"\nError @A sento"<<endl;
					return(-1);
...
void writeToCharacterDevice(char buffer[MSG_SIZE]){
	cout<<"\nWriting to character device"<<endl;
	int cdev_id, dummy;
	// Open the Character Device for writing
	if((cdev_id = open("/dev/MarshallLab6", O_WRONLY)) == -1) {
		cout<<"Cannot open device "<<CHAR_DEV<<endl;
		exit(1);
	}
	cout<<"\nBuffer size: "<<sizeof(buffer)<<endl;
...
void* readFromCharacterDevice(void*){
	...
	if((cdev_id = open("/dev/MarshallLab6", O_RDONLY)) == -1){
		cout<<"Cannot open the device "<<CHAR_DEV<<endl;
		exit(1);
	}
	while(1){
		usleep(100);
		if(read(cdev_id, broadcast, sizeof(broadcast))){
