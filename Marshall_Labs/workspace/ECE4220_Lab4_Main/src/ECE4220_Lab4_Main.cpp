...
void* collection(void*){
	...
	while(1){
		...
		for(int i = e_vector.size() - 1; i >= 0; i--){
...
[NewGps], [OldGps], [EventNewest], ... , [EventOldest]
...
void* interpolation(void*){
		//Variables
		double gpsTimeOld, gpsTimeNew, gpsTimeDiff;
		...
		while(1){
			...
			if(!toBeInterpolated.empty()){
				...
				if(array.size() > 2){
					cout<<"\nPRINTING DATA"<<endl;
					gpsTimeNew = (double)(array[0].time.tv_sec) * 1000000000 + (double)(array[0].time.tv_nsec);

					gpsTimeOld = (double)(array[1].time.tv_sec) * 1000000000 + (double)(array[1].time.tv_nsec);

					gpsTimeDiff = (gpsTimeNew - gpsTimeOld) / 1000000000;

					dataSlope = (array[0].value - array[1].value)/(gpsTimeDiff);

					intercept = array[0].value - ((gpsTimeNew/1000000000) * dataSlope);
					cout<<"INTERCEPT: "<<intercept<<endl;
					for(int i = array.size() - 1; i >= 2; i--){
						eventTime = ((double)(array[i].time.tv_sec) * 1000000000 + (double)(array[i].time.tv_nsec)) / 1000000000;
						eventData = (eventTime * dataSlope) + intercept;
						cout<<"Time : "<<eventTime<<" Data : "<<eventData<<endl;
